...
class ServerIntermediary {
                    ...
                    for (; i < names.length; i++) {
                        ...
                        if (debug) {
                            logger.debug("addNotificationListener(ObjectName,NotificationFilter)",
                                    "connectionId=" + clientId + " unwrapping filter with target extended ClassLoader.");
                        }
 ...
 /* Replace unserializable notifications in the NotificationResult by
    ...
    private NotificationResult purgeUnserializable(NotificationResult nr) {
        ...
        for (TargetedNotification tn : tns) {
            ...
            try {
                ...
                Notification goodNotif = new JMXConnectionNotification(notifType, badNotif.getSource(),
                        clientId, badNotif.getSequenceNumber(), notifMessage, ONE_LONG);
                ...
                trialnr = new NotificationResult(0, 0,
    ...
    private final GenericServerCommunicatorAdmin serverCommunicatorAdmin;
