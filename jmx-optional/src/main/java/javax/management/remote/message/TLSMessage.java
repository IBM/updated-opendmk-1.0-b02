...
/**
 * <p>
 * Handshake message between client and server to set up the TLS handshake.</p>
 *
 * <p>
 * This class represents the handshake messages exchanged between the client and
 * the server to agree on the initiation of the TLS handshake.</p>
 *
 * <p>
 * When the client sends the TLSMessage(READY), it must not send any further
 * traffic on the underlying transport service until a corresponding reply
 * message, either TLSMessage(PROCEED) or an
 * {@link HandshakeErrorMessage indication} message, is received. When the
 * client receives a TLSMessage(PROCEED) in reply to its TLSMessage(READY) it
 * immediately begins the underlying negotiation process for TLS transport
 * security.
 * <p>
 * When the server receives the TLSMessage(READY), it must not send any further
 * traffic on the underlying transport service until it generates a
 * corresponding reply. If the server decides to allow TLS transport security
 ...
 * <ul>
 * <li><b>READY</b> : used by a client to indicate that it is ready to start the
 * TLS handshake.</li>
 * <li><b>PROCEED</b> : used by a server to indicate that it allows the client
 * to proceed with the TLS handshake.</li>
 * </ul>
