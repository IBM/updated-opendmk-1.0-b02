...
/**
 * <p>
 * The client end of a JMXMP Connector. An object of this type can be used to
 ...
 * a {@link JMXServiceURL} that has protocol type <code>jmxmp</code>.</p>
 */
public class JMXMPConnector extends GenericConnector
    ...
    /**
     * <p>
     * Constructs a JMXMP Connector client that can make a connection to the
     * connector server at the given address. This constructor is equivalent to {@link
     ...
     * @exception MalformedURLException if <code>address</code> is not a valid
     * URL for the JMXMP connector.
     ...
     * <p>
     * Constructs a JMXMP Connector client that can make a connection to the
     * connector server at the given address.</p>
     *
     * @param address the address of the connector server to connect to.
     *
     * @param env the environment parameters controlling the connection. This
     * parameter can be null, which is equivalent to an empty map. The provided
     ...
     * @exception IOException if the connector cannot work for another reason.
     */
    public JMXMPConnector(JMXServiceURL address, Map<String, ?> env) throws IOException {
        super(env);
        this.env = new HashMap<>((env == null) ? Collections.emptyMap() : env);
     ...
     * representation that is easy for a person to read.</p>
     *
    ...
    private final transient Map<String, Object>  env;
