...
public class AdminClient implements ClientAdmin {
    ...
    public AdminClient(Map<String, Object> env) {
        this.env = (env != null) ? env : (Map<String, Object>) Collections.EMPTY_LIST;
    }

    public MessageConnection connectionOpen(MessageConnection mc) throws IOException {

        boolean sendError = true;
        HandshakeErrorMessage error;
     ...
     * Otherwise, the default profile selection algorithm implemented works as
     * follows:
     * <p>
     * Upon reception of the HandshakeBeginMessage the client verifies that the
     * profiles he wants to use, i.e. the ones he specified in the environment
     * map through the jmx.remote.profiles property are all present in the
     * server's supported profiles list. If false, the client sends a
     ...
     * selected profiles following the order specified in the client's profile
     * list.
     */
    private List<String> selectProfiles(String serverProfiles) throws Exception {
        ...
        boolean clientFlag = (clientProfiles == null || clientProfiles.isEmpty());
        ...
        if ((serverProfilesListSize < clientProfilesListSize)
                || (!new HashSet<>(serverProfilesList).containsAll(clientProfilesList))) {
            throw new IOException("The server supported profiles "
                    + serverProfilesList + " do not match the client required profiles " + clientProfilesList + ".");
        }
    ...
    private final List<ProfileClient> profilesList = new ArrayList<>();
