...
public class SASLClientHandler implements ProfileClient {
    ...
    // Constructors
    //-------------
    public SASLClientHandler(String profile, Map<String, Object> env) {
        this.env = env;
    ...
    //----------------
    /**
     * Returns an array of SASL mechanisms given a string of space separated
     * SASL mechanism names.
     *
     * @param str The non-null string containing the mechanism names
     * @return A non-null array of String; each element of the array contains a
     * single mechanism name.
     */
    private static String[] getSaslMechanismNames(String str) {
        StringTokenizer parser = new StringTokenizer(str);
        List<String> mechanisms = new ArrayList<>(10);
        while (parser.hasMoreTokens()) {
            mechanisms.add(parser.nextToken());
        }
        String[] mechs = new String[mechanisms.size()];
        for (int i = 0; i < mechanisms.size(); i++) {
            mechs[i] = mechanisms.get(i);
        }
        return mechs;
    }
