...
public class TLSServerHandler implements ProfileServer {
    ...
    // Constructors
    // -------------
    public TLSServerHandler(String profile, Map<String, Object> env) {
    ...
    public void initialize(MessageConnection mc, Subject s) throws IOException {
        ...
        if (needClientAuth != null) {
            ts.setNeedClientAuth(Boolean.parseBoolean(needClientAuth));
        }
        if (logger.traceOn()) {
    ...
    public void consumeMessage(ProfileMessage pm) throws IOException {
        if (!(pm instanceof TLSMessage)) {
            throw new IOException("Unexpected profile message type: " + (pm != null ? pm.getClass().getName() : "null"));
        }
                ...
                if (certificate != null && certificate[0] != null && certificate[0] instanceof X509Certificate cert) {
                    ...
                    if (bundledJSSE) {
                        try {
                            ...
                            logger.trace("activate", mh + "Got exception building the "
                                    + X500_PRINCIPAL + " from the principal stored in the client's certificate.");
