...
public abstract class ClientNotifForwarder {
    ...
    protected ClientNotifForwarder(Map<String, Object> env) {
        this(null, env);
    }

    protected ClientNotifForwarder(ClassLoader defaultClassLoader, Map<String, Object> env) {
    ...
    public synchronized ListenerInfo[] getListenerInfo() {
        return infoList.values().toArray(new ListenerInfo[0]);
    }
    ...
    public synchronized void postReconnection(ClientListenerInfo[] listenerInfos) throws IOException {
        ...
        for (ClientListenerInfo listenerInfo : listenerInfos) {
            if (trace) {
                logger.trace("addNotificationListeners", "Add a listener at " + listenerInfo.getListenerID());
            }
        ...
        if (currentFetchThread == Thread.currentThread()) {
            ...
            try {
                ...
                final String msg = "Failed to register a listener to the mbean server: the client will not do clean when an MBean "
                        + "is unregistered";
                if (logger.traceOn()) {
...
// -------------------------------------------------
    ...
    private class NotifFetcher implements Runnable {
        ...
        void dispatchNotification(TargetedNotification tn, Integer myListenerID, Map<Integer, ListenerInfo> listeners) {
            final Notification notif = tn.getNotification();
            final Integer listenerID = tn.getListenerID();
...
// -------------------------------------------------
    ...
    /*
     * Called to decide whether there is a need to start a thread for fetching notifs.
     * <P>The parameter reconnected will decide whether to initialize the clientSequenceNumber,
     * initializing the clientSequenceNumber means to ignore all notifications arrived before.
     ...
     * notifications arrived before its registry.
     */
...
// -------------------------------------------------
     ...
     * true by the method <code>preReconnection</code>, and set false by
