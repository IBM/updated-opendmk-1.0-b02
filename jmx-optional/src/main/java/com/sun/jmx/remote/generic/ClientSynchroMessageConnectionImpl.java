...
public class ClientSynchroMessageConnectionImpl implements ClientSynchroMessageConnection {
    ...
    public ClientSynchroMessageConnectionImpl(MessageConnection mc, SynchroCallback cb, Map<String, ?> env) {
            ...
            if (state == UNCONNECTED) {
                ...
                if (logger.traceOn()) {
                    String s = "Wakeup the threads which are waiting a response "
                            + "from the server to inform them of the connection failure.";
    ...
    public Message sendWithReturn(Message msg) throws IOException {
        ...
        if (msg instanceof NotificationRequestMessage) {
            ...
            while (true) {
                ...
                // and it will not be modified anymore. Synchronizing on mwrapper
                // is no longer needed.
                //
                if (!mwrapper.got) {
                    if (!isTerminated()) {
    ...
    public void close() throws IOException {
        ...
        synchronized (stateLock) {
            ...
            // clean
            if (logger.traceOn()) {
                logger.trace("close", "Clean all threads waiting their responses.");
            }
            ...
            try {
                ...
                while (!stopped()) {
                    ...
                    if (msg instanceof NotificationResponseMessage) {
                        ...
                        synchronized (waitingList) {
                            mwrapper = waitingList.get(((MBeanServerResponseMessage) msg).getMessageId());
                        }
...
//----------------------------------------------
    ...
    /**
     * Maps message id to ResponseMsgWrapper, locked at itself when the map is
     * updated. A ResponseMsgWrapper is used to wait for response for given
     * request. Synchronizing on it to do wait/notify
     */
    private final transient Map<Long, ResponseMsgWrapper> waitingList = new HashMap<>();

    // notif stuff.
    private transient Message notifResp = null;

    /**
     * Controls access to notifResp field; used in wait/notify so waiting thread
     * can be informed when a notifResp is set.
     */
    ...
    /**
     * Used to control access to the state variable, including ensuring that
     * only one thread manages state transitions.
     */
