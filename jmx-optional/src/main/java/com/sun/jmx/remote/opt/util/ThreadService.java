...
public class ThreadService {
    ...
    public void handoff(Runnable job) {
        ...
        synchronized (lock) {
            ...
            // should ask more threads immediately
                    ...
                    if (jobList.isEmpty()) {
                        if (total > min) {
                            if (idle == 0) { // keep one with timeout
                                ...
                                while (jobList.isEmpty() && remainingTime > 0) {
                                    try {
                            ...
                            while (!terminated && jobList.isEmpty()) {
                                try {
                                    lock.wait();
...
// private stuff
    ...
    private final List<Runnable> jobList = new ArrayList<>();
